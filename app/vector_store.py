...
class OpenRouterEmbedding:
    """Embedding client via OpenRouter API."""
    ...
    def __init__(self, model: str = "openai/text-embedding-3-small", api_key: Optional[str] = None):
        ...
        logger.info("OpenRouter embedding: %s", model)
            ...
            for attempt in range(self.MAX_RETRIES):
                try:
                    ...
                    if attempt < self.MAX_RETRIES - 1:
                        wait = 2 ** (attempt + 1)
                        logger.warning("Embedding batch %d attempt %d failed: %s, retrying in %ds", i, attempt + 1, e, wait)
                        time.sleep(wait)
                    else:
                        logger.error("Embedding failed for batch %d after %d attempts: %s", i, self.MAX_RETRIES, e)
                        raise
